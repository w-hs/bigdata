...
In den \textbf{transactions} (s. Tabelle \ref{tab:transactions}) stehen die Einkäufe der Kunden.
Die \textbf{offers} (s. Tabelle \ref{tab:offers}) enthalten Gutscheine, die an den Kunden verteilt wurden.
Die \textbf{train\_history} (s. Tabelle \ref{tab:trainhistory}) enthält Informationen darüber, wann den Kunden die Coupons angeboten wurden.
Die \textbf{test\_history} entspricht der \textbf{train\_history} mit dem Unterschied, dass die beiden Felder "`repeattrips"' und "`repeater"' fehlen. Diese sind absichtlich nicht enthalten, da eine Vorhersage über diese Werte getroffen werden soll.
Die Tabelle \textbf{submissions} (s. Tabelle \ref{tab:submissions}) enthält die Ergebnisse, die wir bei Kaggle einreichen.
